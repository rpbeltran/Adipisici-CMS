...
class Data_Layer {
    ...
    createDataForm( name, schema ) {  // Promise
        ...
        // Handle Nonstandard Data Types
        for( let k in schema ) {
            if ( schema.hasOwnProperty(k) ) {
                if( schema[k].hasOwnProperty("type") ) {
                    if ( typeof(schema[k].type) == "string" ) {
                        schema[k].editor_type = schema[k].type;
                        schema[k].type = String;
                        continue;
                    }
                }
                schema[k].editor_type = schema[k].type.name;
            }
         }


        this.data_templates[name] = schema;
        this.data_schemes[name] = this.mongoose.Schema( schema );
        this.data_models[name]  = this.mongoose.model( name, schema );
        return this.data_models[name];
    }
